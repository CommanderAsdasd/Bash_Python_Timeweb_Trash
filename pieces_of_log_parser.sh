...
`
{ logs n vh48 & logs n vh50 & logs n bitrix152; } | egrep "americanas|banking|Canada|blackfriday|visa|cheap|jackson|flight|ofertas|carnaval|walmart|cadastro|garantia|Galaxy|smartphone" > temp.txt; tac temp.txt | egrep -v " 404 | 403 | 500 | 503 |*error_domain.*" | awk '{print $6}' | egrep -o "[a-zA-Z]{2}[0-9]{5}" | sort | uniq > temp`date +%H-%M`.txt
